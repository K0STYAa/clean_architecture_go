...
import (
	...
	"forgoproject/pkg/tools/transaction"
	"forgoproject/pkg/type/columnCode"
	"forgoproject/pkg/type/context"
	log "forgoproject/pkg/type/logger"
	"forgoproject/pkg/type/queryParameter"
	"forgoproject/services/contact/internal/domain/contact"
	"forgoproject/services/contact/internal/repository/storage/postgres/dao"
	"forgoproject/services/contact/internal/useCase"
)

var mappingContact = map[columnCode.ColumnCode]string{
	"id":          "id",
	"fullName":    "full_name",
...
func (r *Repository) listContactTx(ctx context.Context, tx pgx.Tx, parameter queryParameter.QueryParameter) ([]*contact.Contact, error) {
	...
	if len(parameter.Sorts) > 0 {
		builder = builder.OrderBy(parameter.Sorts.Parsing(mappingContact)...)
	} else {
